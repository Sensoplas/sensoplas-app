...
export default class BleGetter {
  ...
  constructor(private macAddress: string) {
  ...
  private async connect(): Promise<boolean> {
    ...
    return new Promise(function (resolve, reject) {
      _this.bluetoothSerial.connect(_this.macAddress).subscribe(
        (x) => {
          //When a byte is returned. We don't actually care what it returned it's just nice to know it's trying its best
          _this.connectionStatus = true;
          resolve(true);
        },
        (err) => {
          //Failed read, maybe it was a unexpected disconnect maybe it was a small to medium sized explosion, (which i guess would cause an unexpected disconnect).
